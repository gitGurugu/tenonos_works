...
void trap_el1_sync(struct __regs *regs, __u64 far)
{
	...
	if (trap < AARCH64_TRAP_MAX) {
		...
		switch (rc) {
		case UK_EVENT_HANDLED_CONT:
			// fixme: UK_EVENT_HANDLED_CONT means continue to call
			// next handler, but unikraft doesn't do so, maybe we
			// should handle this.
			uk_pr_debug(
			    "Event %s handled, continue to call next handler",
			    _trap_table[trap].str);
			return;
		case UK_EVENT_HANDLED:
			uk_pr_debug("Event %s handled\n",
				    _trap_table[trap].str);
			return;
		case UK_EVENT_NOT_HANDLED:
			uk_pr_debug("Event %s is not handled\n",
				    _trap_table[trap].str);
			break;
		default:
			uk_pr_crit("Event handler for %s returned error: %d\n",
				   _trap_table[trap].str, rc);
			break;
		}
	}
...
/*
 * Fixme：some arm platforms don't use gic as irq driver, here we just drop the
 * logic by micro config, while drivers should be encapsulated in the controller
 * library so that the caller is not aware of the difference
 */
void __weak trap_el1_irq(struct __regs *regs)
{
#ifdef CONFIG_ARM64_FEAT_MTE
