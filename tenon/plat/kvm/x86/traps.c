...
static void gdt_init(__lcpuid id)
{
	...
	cpu_gdt64[id][GDT_DESC_CODE].raw = GDT_DESC_CODE64_VAL;
	cpu_gdt64[id][GDT_DESC_DATA].raw = GDT_DESC_DATA64_VAL;
...
static void tss_init(__lcpuid id)
{
	...
	cpu_tss[id].ist[2] =
		(__u64) &cpu_crit_stack[id][sizeof(cpu_crit_stack[id])];

	tss_desc = (void *) &cpu_gdt64[id][GDT_DESC_TSS_LO];
	tss_desc->limit_lo	= sizeof(cpu_tss[id]);
	tss_desc->base_lo	= (__u64) &(cpu_tss[id]);
	tss_desc->base_hi	= (__u64) &(cpu_tss[id]) >> 24;
...
{
	gdt_init(lcpu_get_current()->id);
	tss_init(lcpu_get_current()->id);
	idt_init();
}
